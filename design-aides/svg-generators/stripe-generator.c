...
static void print_usage(const struct opts *opts)
{
	...
	fprintf(stderr,
"%s - Generates SVG file of camouflage stripes.\n"
"Usage: %s [flags]\n"
"Option flags:\n"
	...
	const struct grid_position *pos)
{
	...
	log("%s: %u nodes at {%u,%u} => {%f,%f}\n",
		...
		stripe_offset.x, stripe_offset.y);

	svg_open_path(out_stream, stripe_id, color, NULL);
	...
	for (node = 0, point_p.angle = 0; node < node_count; node++) {
		...
		final.x = point_c.x + stripe_offset.x;
		final.y = point_c.y + stripe_offset.y;
